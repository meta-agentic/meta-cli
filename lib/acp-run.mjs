...
child.on('exit', (code, signal) => {
  for (const [, p] of pending) {
    p.rej(new Error(`agent exited (${code ?? signal ?? 'unknown'}) before reply`));
  }
  pending.clear();
});
...
const HANDSHAKE_MS = 20_000; // initialize / session/new|load must not wait forever
...
async function main() {
  ...
  let sessionId = prior;
  if (prior) {
    try { await request('session/load', { sessionId: prior, cwd: args.cwd, mcpServers: [] }, HANDSHAKE_MS); }
    catch (e) { process.stderr.write(`acp-run: session/load failed (${e.message}); starting fresh\n`); sessionId = null; }
  }
  if (!sessionId) {
    const r = await request('session/new', { cwd: args.cwd, mcpServers: [] }, HANDSHAKE_MS);
    sessionId = r?.sessionId;
    if (!sessionId) fail(1, 'agent did not return a sessionId');
  }

  const promptMs = args.timeout > 0 ? args.timeout * 1000 : 0;
